...
export default function Home() {
  ...
  function handleSubmit(data: MyForm) {
    console.log(data)
  }
        ...
        <Form {...form}>
          <form className="flex flex-col gap-6" onSubmit={form.handleSubmit(handleSubmit)}>
            <FormField
              ...
              name="username"
              rules={{
                required: "Введи имя (минимум 2 буквы)",
                minLength: {value: 2, message: "Введи имя (минимум 2 буквы)"}
              }}
            ...
            <div className="flex items-start gap-4 sm:flex-row flex-col">
              <FormField
                ...
                rules={{
                  validate: (value) => {
                    ...
                    if (!value && !phone) return 'Введи никнейм или ссылку на профиль';
              ...
              />
              <span className="font-medium text-sm self-end mb-2 sm:inline hidden">или</span>
              <FormField
                control={form.control}
                name="phonenumber"
                rules={{
                  validate: (value) => {
                    const phone = form.getValues('telegram');
                    if (!value && !phone) return 'Введи номер телефона';
                    form.clearErrors('telegram');
                    form.clearErrors('phonenumber');
                  },
                }}
                render={({field, fieldState}) => (
                  <FormItem className="w-full">
                    <div className="flex flex-col gap-0.5">
                      <FormLabel className="font-medium text-sm" htmlFor="phonenumber">
                        Номер
                      </FormLabel>
                      <FormControl>
                        <Input
